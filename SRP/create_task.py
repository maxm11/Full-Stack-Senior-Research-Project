...
def create_task(project, queue, location, task_handler, payload=None, in_seconds=None):
    ...
    creds_path = dir_path + '/libs/creds.json'

    credentials = service_account.Credentials.from_service_account_file(creds_path)
    # Create a client.
    client = googleapiclient.discovery.build('cloudtasks', 'v2beta2', credentials=credentials)

    # Construct the request body.
    url = '/{}'.format(task_handler)
    body = {
        'task': {
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--task_handler',
        help='The URL of the task handler.',
        required=True,
    )
    ...
    create_task(
        args.project, args.queue, args.location, args.task_handler,
